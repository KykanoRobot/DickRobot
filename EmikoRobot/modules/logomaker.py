...
async def logo_gen(event):
    ...
    if not bg_:
        pics = []
        async for i in ubot2.iter_messages(
            "@KaynoLogoPack", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Dezky's Robot](https://t.me/RobotDick_bot)",
...
async def logo_(event):
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Kayno Robot](https://t.me/KaynoRobot_bot)",
            force_document=False,
        )
        os.remove(flnme)
        await xx.delete()
